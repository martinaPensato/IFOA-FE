...
*/
 function parolaUnita(stringa1, stringa2) {
  let primiDue = stringa1.slice(0,2);
  let ultimiTre= stringa2.slice(-3);
  let risultato= primiDue.concat(ultimiTre).toUpperCase();
  return risultato;
 }

 let stringa1 = "Camaleonte";
 let stringa2 = "Canarino";
 let risultatoConcat = parolaUnita(stringa1, stringa2);
 console.log(risultatoConcat);

/* ESERCIZIO 2 (for)
  Scrivi una funzione che torni un array di 10 elementi; ognuno di essi deve essere un valore random compreso tra 0 e 100 (incluso).
*/
function arrayNCasuale() {
  let arrayCasuale = [];
  for (let i = 0; i < 10; i++) { 
...
let arrayCasuale = arrayNCasuale();
...
let valoriPari = trovaValoriPari(arrayCasuale);
...
let risultatoSomma = sommaNumeri(arrayCasuale);
console.log(risultatoSomma);

/* ESERCIZIO 5 (reduce)
  Scrivi una funzione per sommare i numeri contenuti in un array
*/
function sommaNumeriReduce(array) {
  const somma = array.reduce((acc, numero) => acc + numero, 0);
  return somma;
}

let risultatoSommaReduce = sommaNumeriReduce(arrayCasuale);
console.log(risultatoSommaReduce); 
...
let stre=["Camaleonte","canarino","cane"];
...
const titoli = movies.map ((movies2) => `Titolo: ${movies2.Title}`)
