...
const changePContent = function (changedP) {
  const paragrafi = document.querySelectorAll("div > p");
  paragrafi.forEach((paragrafo) => {
    paragrafo.textContent = "Paragrafi Nuovi";
  });
};
...
const addToTheSecond = function () {
  ...
  lista.innerText = "Questo è il quarto elemento che ho aggiunto io";
};
...
const addParagraph = function () {
  const primoDivP = document.getElementById("primoDiv");
  const paragraph = document.createElement("p");
  const nuovoP = primoDiv.appendChild(paragraph);
  paragraph.innerText = "Questo è il p che ho aggiunto io";
};
...
const paintItGreen = function () {
  const liste2 = document.querySelectorAll("ul");
